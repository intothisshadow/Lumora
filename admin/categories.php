    ...
    if ($act === 'save') {
        ...
        if ($edit_id > 0) {
            ...
            LumoraDB::update('categories',
                ['name' => $name, 'description' => $desc, 'parent_id' => $parent_id, 'pos' => $pos],
                'id = ?', [$edit_id]
            );
...
if (empty($all_cats)) {
    ...
    foreach ($all_cats as $c) {
        ...
        $del_conf = "Are you sure you want to delete '" . addslashes($c['name']) . "'? Child items will be moved to parent.";
        $rows .= <<<HTML
<tr>
  <td>{$name_h}</td>
  <td>{$parent_h}</td>
...
<div class="d-flex justify-content-between align-items-center mb-3">
  <span class="text-muted">{$s_total} categories</span>
...
<div class="table-responsive">
  <table class="table table-hover lum-adm-table align-middle">
    <thead>
      <tr>
        <th>Name</th><th>Parent</th><th>Pos</th><th></th><th></th>
      </tr>
    </thead>
    <tbody>
      {$rows}
...
// Fix string used before assignment above
$content = str_replace('{$s_total}', (string)$s_total, $content);
